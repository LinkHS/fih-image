	...
	for(nY=0; nY<m_nHei; nY++)
	{
		for(nX=0; nX<m_nWid; nX++)
		{
			...
			for(nItersY=-nSizes/2; nItersY<nSizes/2+1; nItersY+=2)
			{
				...
				{
					nYmin = std::max(0, (nY+nItersY) - nSizes/2);
					nYmax = std::min(m_nHei, (nY+nItersY) + nSizes/2+1);
					nXmin = std::max(0, (nX+nItersX) - nSizes/2);
					nXmax = std::min(m_nWid, (nX+nItersX) + (nSizes)/2+1);
				
					nNewX = std::min(std::max(nX+nItersX, 0), m_nWid-1);
					nNewY = std::min(std::max(nY+nItersY, 0), m_nHei-1);
