...
public:
	...
	void	HazeRemoval(cv::Mat& mSrc, IplImage* imOutput, int nFrame);
	void	ImageHazeRemoval(cv::Mat& mSrc, cv::Mat& mDst);	
	...
	void	AirlightSerachRange(cv::Point pointTopLeft, cv::Point pointBottomRight);
...
private:
	...
	void	IplImageToIntColor(cv::Mat& mSrc);
	void	IplImageToIntYUV(IplImage* imInput);
	
	// dehazing.cpp
	void	AirlightEstimation(cv::Mat& mSrc);
	void	RestoreImage(cv::Mat& mSrc, cv::Mat& mDst);
