...
void YUYVSkinSmooth(uchar* pYUYV, int width, int height)
{
#if 0
    CaptureProcess_FaceBeauty(pYUYV, width, height);
#elif 0
    CaptureProcess_Dehazing(pYUYV, width, height);
#elif 0
    CaptureProcess_Denoising1(pYUYV, width, height);
#elif 0
    CaptureProcess_Denoising2(pYUYV, width, height);
#elif 0
    CaptureProcess_LocalHistEqual_noSaturation(pYUYV, width, height);
#elif 0
    //img.convertTo(saturated, CV_8UC1, scale, saturation);
#elif 1
    CaptureProcess_LocalHistEqual(pYUYV, width, height);
#endif
}
...
// FIH Face Beauty in capture, get Face Detected info from MTK
void FIH_FBCapture_GetFDInfo_Callback(void* pFaceInfo)
{
    int i = *(int*)pFaceInfo;
}
...
void VideoLSFilter_Init(u32 height, u32 width)
{
    g_FDManager = new MTK_FDManager;
    //g_videoLSFilter = new VideoLSFilter;
...
void VideoLSFilter_Uninit(void)
{
    ...
    //delete g_videoLSFilter;
    //g_videoLSFilter = NULL;

    g_videoLSFilter.uninit1();
}

void VideoLSFilter_Run(u8* pYUV, void* pUserData)
{
    int faceRects[3][4];
    int frameH, frameW;
